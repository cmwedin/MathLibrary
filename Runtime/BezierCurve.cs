...
namespace SadSapphicGames.MathLibrary {
    public class BezierCurve {
        ...
        private Func<float, double> GetBernsteinBasisPolynomial(int n,int v) { //? n is the degree of the bezier curve, v is an element of [0,n]
            if(v > n || v < 0) throw new Exception("illegal argument");
            Func<float, double> output = t => (double)Functions.BinomialCoefficient(n,v)*Math.Pow(t,v)*Math.Pow(1-t,n-v);
            return output;
        }
    }
}
